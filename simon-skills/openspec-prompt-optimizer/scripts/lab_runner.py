#!/usr/bin/env python3
"""Run one Pre-Spec prompt variant through a provider or fixture harness."""
...
from evaluate_prompt_run import evaluate  # noqa: E402
...
def provider_metadata_from_raw(raw: dict[str, object]) -> dict[str, object]:
    usage = raw.get("usage")
    return {
        "model": raw.get("model"),
...
def call_perplexity_agent(request_body: dict[str, object]) -> dict[str, object]:
    ...
    if not api_key:
        raise RuntimeError("PERPLEXITY_API_KEY or VITE_PERPLEXITY_API_KEY is required for provider runs.")
...
def main() -> int:
    ...
    parser.add_argument("--case", type=Path, required=True, help="JSON case with id/companyName/variables")
    ...
    if args.surface == "fixture":
        ...
        raw_response: dict[str, object] = {
            ...
            "request": request_without_secret,
        }
    else:
        try:
    ...
    provider_metadata = provider_metadata_from_raw(raw_response)

    parsed, metrics = evaluate(response_text, args.expected_object, args.expected_host)
    metrics["duration_ms"] = duration_ms
    metrics["model"] = provider_metadata["model"]
    metrics["token_usage"] = provider_metadata["token_usage"]
    metrics["provider_error"] = error
    metrics["case_id"] = case_id
    metrics["variant_id"] = args.variant_id

    request_path = artifact_base.with_name(artifact_base.name + "__01_request.json")
    prompt_path = artifact_base.with_name(artifact_base.name + "__02_rendered-prompt.txt")
    raw_path = artifact_base.with_name(artifact_base.name + "__03_raw-response.json")
    text_path = artifact_base.with_name(artifact_base.name + "__04_response-text.txt")
    parsed_path = artifact_base.with_name(artifact_base.name + "__05_parsed-output.json")
    metrics_path = artifact_base.with_name(artifact_base.name + "__06_metrics.json")

    request_path.write_text(json.dumps(request_without_secret, ensure_ascii=False, indent=2), encoding="utf-8")
    ...
    result = {
        ...
        "token_usage": provider_metadata["token_usage"],
        "max_output_tokens": args.max_output_tokens,
        "request_without_secret": request_without_secret,
        "prompt": {
